class Account < ApplicationRecord
  ...
  belongs_to :owner, class_name: "User", optional: true  # nil = shared account
  ...
  scope :shared, -> { where(owner_id: nil) }
  scope :for_user, ->(user) { where(owner: [user, nil]) }  # User's own + shared
  ...
  # Display owner name or "Shared" for nil owner
  def owner_name
    owner&.first_name || "Shared"
  end
